    ...
    <div>
        ...
        <div class="form-box">
            <el-form ref="form" :model="form" label-width="100px" enctype="multipart/form-data" method="post">
                ...
                <el-form-item label="景区属性" prop="logicName" :rules="[{ required: true, message: '景区属性不能为空'}]">
                    <el-select v-model="form.logicName" placeholder="请选择" @change="handleChange0">
                ...
                <el-form-item label="景区二维码" prop="file">
                    <el-upload class="upload-demo" ref="upload" :action=urlss :on-preview="handlePictureCardPreview" :on-remove="handleRemoves" :on-success="success" :limit=1
                        :data=lists list-type="picture-card" :file-list="fileLists" :auto-upload="true" accept=".jpg,.png,.jpeg,.gif,.bmp">
                        <el-button slot="trigger" size="small" type="primary">选取文件</el-button>
                        <!-- <el-button style="margin-left: 10px;" size="small" type="success" >开始上传图片</el-button> -->
...
<script>
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                urls: common.apidomain + "/view/addImages.action", //景区图片文件地址
        ...
        methods: {
            ...
            handleRemove(file, fileList) {   //景区图片删除
                console.log(file, fileList);
                axios.get(common.apidomain + "/view/deleteImages.action?random_no="+this.lists.random_no+"&deleteURL="+file.name,).then((res) => {
                    console.log(res);
                })
            },
            handleRemoves(file, fileList) {   //景区二维码删除
                console.log(file, fileList);
                axios.get(common.apidomain + "/view/deleteViewImg.action?random_no="+this.lists.random_no+"&deleteURL="+file.name,).then((res) => {
                    console.log(res);
                })
            },
            ...
            axios.get(common.apidomain + "/view/editUI.action?id=" + this.$route.query.id).then((res)=> { 
                ...
                this.fileList = res.data.data.pic; //景区图片的回显 
                this.fileLists = res.data.data.view_img_pic; //景区二维码的回显 
    ...
    handleChange1(value) {
        ...
        onSubmit(formName) {
            ...
            console.log(this.form.logicId)
            console.log(this.form.staffId)
            //景区属性
            if(this.form.logicId==undefined){
                this.form.logicId=this.form.logic;
            }else{
                this.form.logicId
            }   
            //业务人员
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    axios.post(common.apidomain + "/view/addUpdate.action?name=" + this.form.name +
                        ...
                        "&staffId=" + this.form.staffId +"&logic=" + this.form.logicId + "&province=" + this.form.province +
                        ...
                        "&busMessage=" + this.form.busMessage + "&tinyPrice=" + this.form.tinyPrice+ "&tag1=" + this.form.tag1+ "&tag2=" + this.form.tag2+ "&tag3=" + this.form.tag3+ "&tag4=" + this.form.tag4 + "&selfRoute=" + this.form.selfRoute +
